...
function layout:Initialize(frame)
    ...
    frame.HealthBar:SetStatusBarTexture("Interface\\AddOns\\sArena Flat\\statusbar")

    frame.PowerBar:SetStatusBarTexture("Interface\\AddOns\\sArena Flat\\statusbar")
    ...
    -- This prevents Arena 2 from having different size than Arena 1&3
    frame.ClassIcon:SetSize(43, 43)  -- Fixed theme size for all frames
    frame.ClassIcon:SetTexCoord(0.07, 0.93, 0.07, 0.93)
    frame.ClassIcon:Show()

    -- DEBUG: Check runtime class icon bounds for all frames
    ...
    -- Wait a frame then print actual runtime values
    C_Timer.After(0.1, function()
        if frame.ClassIcon then
            local fw, fh = frame:GetSize()
            local iw, ih = frame.ClassIcon:GetSize()
            local ft, fb = frame:GetTop(), frame:GetBottom()
            local it, ib = frame.ClassIcon:GetTop(), frame.ClassIcon:GetBottom()
            print("|cffFFFF00[SIMPLE CORE]|r", frameName, "(", frameIndex, ") frame:", fw.."x"..fh, "icon:", iw.."x"..ih, "frameT:", ft, "iconT:", it, "frameB:", fb, "iconB:", ib)
        end
    end)
    ...
    -- This fixes player name sliders not working with Simple Core theme
    ...
    if AC and AC.DB and AC.DB.profile then
        ...
        if AC.ArenaFrameThemes then
            ...
            if general then
                ...
                print(string.format("|cff00FFFF[SIMPLE CORE]|r Using slider values: X=%d, Y=%d", nameX, nameY))
            end
        end
    end
    ...
    f = frame.CastBar
    f:SetStatusBarTexture("Interface\\AddOns\\sArena Flat\\statusbar")
    ...
    if AC and AC.DB and AC.DB.profile then
        if AC.ArenaFrameThemes then
            ...
            if general then
                nameX = (general.playerNameX ~= nil) and general.playerNameX or 0
                nameY = (general.playerNameY ~= nil) and general.playerNameY or 2
                print(string.format("|cff00FFFF[SIMPLE CORE UPDATE]|r Updating name position: X=%d, Y=%d", nameX, nameY))
            end
        end
    end
